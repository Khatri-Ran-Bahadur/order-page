...
if (!@$_REQUEST['submit']) {
//echo " SELECT * FROM orders_orders WHERE order_id = '$order_id' AND payment_status = '0' ";die;
	$sql = @mysqli_query($dbcon," SELECT * FROM orders_orders WHERE order_id = '$order_id' AND payment_status = '0' ");
	...
	echo 'Invalid Request';
		...
		while ($row = @mysqli_fetch_array($sql)) {
			...
			$order_codex = "#".$row['order_id'];
			echo '
			<form method="POST" action="paynow.php">
			<input type="hidden" name="dconfirm" value="2">
			<input type="hidden" name="order_id" value="'.$row['order_id'].'">
			<input type="hidden" name="total_h" value="'.$row['total_h'].'">
			<input type="hidden" name="curr" value="'.$row['curr'].'">
			<input type="hidden" name="order_code" value="'.$order_codex.'">
			<p>You have successfully placed an order with us and the system has created an account for you and sent the LOGIN details to your email address <b>'.$email.'</b>. If the email is not in the inbox, please check your SPAM/JUNK folder and set it as \'Not spam\'.</p>
			<div  class="contentheading">Order Payment</div>
			<p><b>Order :</b> <a target="_BLANK" href="view.php?order_id='.$row['order_id'].'">'.stripslashes($row['topic']).'</a> </p>
			<p><b>Total :</b> '.$row['total_x'].' &nbsp;</p>
			';
	...
	if($dtype == "mail" && $dconfirm == "1") {
		...
		<?php
			...
			$sql2 = @mysqli_query($dbcon,"SELECT * FROM orders_deposits WHERE email = '$email' AND order_id = '$order_id' ");
			if (@mysqli_num_rows($sql2)== 0 ) {
				...
				@mysqli_query($dbcon,"UPDATE orders_deposits SET dtype = '$dtype', amount = '$total_h', status = 'Not Approved', month = '$month', day = '$day', year = '$year', time = '".time()."' WHERE order_id = '$order_id' ");
			}
			...
			<!--	<form name="_xclick" action="<?=$siteurl;?>/order/pp_complete.php" method="post" id="paymentProcessor">-->
		...
		if ($dconfirm == "1") {
			...
			<input type="hidden" name="t" value="Service-related deposit on <?=$companyname;?> Email: <?=$email;?> #<?=$order_id;?>">
